...
export const api = {
  ...
  orient: async (text, topicId, onProgress) => {
    ...
    let content = "This project has no source material yet. The local model only has access to the fictional demo project.";
    if (!isEmptyProject) {
      const { generateDemoAnswer } = await import("./localModel");
      content = await generateDemoAnswer(text, onProgress);
    }
    return settle(
      {
        outcome: "matched",
        ...
        answer: {
          content,
          is_llm: !isEmptyProject,
          generation_note: isEmptyProject
            ? "Static demo: projects created here have no source material and are not sent to the model."
            : "Generated by a small model in this browser from fictional sample content. Check important details.",
          source_files: [],
        },
      },
      300
  ...
  ask: async (topicId, question) => {
    ...
    const { generateDemoAnswer } = await import("./localModel");
    const answer = await generateDemoAnswer(question);
    ...
    const assistantMessage = {
      ...
      is_llm: true,
      source_files: [],
      generation_note: "Generated in this browser using the fictional demo project. Check important details.",
    };
  ...
  askGlobal: async (question) => {
    ...
    return {
      id: 1,
      role: "assistant",
      content: await generateDemoAnswer(question),
      is_llm: true,
      source_files: [],
      generation_note: "Generated in this browser using the fictional demo project. Check important details.",
    };
